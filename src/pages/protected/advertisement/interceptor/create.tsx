...
export default function InterceptorCreate() {
    ...
    const createMutation = useMutation({
        mutationFn: (data: FormData) => api.createInterceptorAd(data),
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            adFormData.append("is_active", formData.is_active.toString());
            ...
            await createMutation.mutateAsync(adFormData);
            setUploadProgress(100);
        } catch (err: any) {
            const msg = err?.response?.data?.errors || err?.response?.data?.message || err?.message;
            setError(typeof msg === "object" ? Object.values(msg).flat().join(", ") : msg || "Failed to create interceptor ad");
        } finally {
            setIsUploading(false);
        }
    };

    const canSubmit = formData.title.trim() && (mediaType === "image" ? !!mediaFile : (!!mediaFile || !!formData.selectedVideo));
    ...
    return (
        <div>
            <Box sx={{ mb: 3, display: "flex", alignItems: "center", gap: 2 }}>
                ...
                <div>
                    <h2 style={{ margin: 0 }}>Create Interceptor Ad</h2>
                    <p style={{ margin: "4px 0 0 0", color: "var(--text-secondary)" }}>Upload an image, GIF, or video to create an interceptor ad</p>
                </div>
            </Box>
            ...
            {isUploading && (
                <Box sx={{ mb: 3 }}>
                    <Typography variant="body2" sx={{ mb: 1 }}>{uploadProgress < 70 ? "Uploading media..." : "Creating interceptor ad..."}</Typography>
                ...
                <Paper sx={{ flex: 1, minWidth: 300, p: 3 }}>
                    ...
                    {mediaPreview ? (
                        ...
                        >
                            ...
                            <Typography variant="body2" sx={{ mt: 1 }}>Drag & drop or click</Typography>
                        ...
                        <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
                            ...
                            <FormControlLabel
                                ...
                                label={
                                    <Box>
                                        ...
                                        <Typography variant="caption" color="text.secondary">
                                            {formData.is_active ? "Ad will be shown to users" : "Ad is disabled and won't be shown"}
                                        </Typography>
                                    </Box>
                                }
                            />
                            ...
                            <Box sx={{ display: "flex", gap: 2, justifyContent: "flex-end", mt: 2 }}>
                                ...
                                <Button type="submit" variant="contained" startIcon={<FaSave />} disabled={isUploading || !canSubmit} sx={{ backgroundColor: "#FF7A00", "&:hover": { backgroundColor: "#E66D00" } }}>
                                    {isUploading ? "Creating..." : "Create Interceptor Ad"}
                                </Button>
                            </Box>
                        </Box>
                    </form>
